...
The deeper course rewrite is in progress. The first three chapters now have 58 rewritten core pages and four supporting readings, with 94 worked examples, expected output and supplied programs to repair or extend. Their coding exercises have 149 individual checks with specific hints and feedback; control-flow and list checks also try alternate inputs and boundary values. Four quizzes explain each wrong choice separately. Fresh list quiz attempts include five original code-blank questions. Optional challenges and later chapters remain in progress. Coverage is tracked in `COURSE-REVISION.md`.
